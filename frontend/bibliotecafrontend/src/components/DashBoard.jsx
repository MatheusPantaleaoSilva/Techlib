...
import CategoryIcon from '@mui/icons-material/Category'; // <--- Adicionei este import
...
const Dashboard = () => {
  ...
  // --- COMPONENTES AUXILIARES DE CARTÃO ---
    ...
    <Paper elevation={3} sx={{ p: 3, borderRadius: 2, height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box display="flex" flexDirection="column" gap={2} mt={2} sx={{ flexGrow: 1 }}>
        {usuario.role === "FUNCIONARIO" ? (
          <>
            {/* Botões de Gestão */}
            <Button variant="contained" startIcon={<PeopleIcon />} component={Link} to="/pessoas" fullWidth>
              Gerenciar Pessoas
            </Button>
            <Button variant="contained" color="secondary" startIcon={<AssignmentIcon />} component={Link} to="/emprestimos" fullWidth>
              Gerenciar Empréstimos
            </Button>
            <Button variant="contained" color="warning" startIcon={<CategoryIcon />} component={Link} to="/categorias" fullWidth>
              Gerenciar Categorias
            </Button>
            <Button variant="contained" color="primary" startIcon={<StarIcon />} component={Link} to="/indicacoes" fullWidth sx={{ bgcolor: '#9c27b0', '&:hover': { bgcolor: '#7b1fa2' } }}>
            ...
            {/* Botão de Consulta */}
            <Button variant="outlined" startIcon={<MenuBookIcon />} component={Link} to="/livros" fullWidth>
              Consultar Acervo
            </Button>
          </>
        ) : (
          <>
            ...
            <Button variant="outlined" component={Link} to="/perfil" fullWidth>
              Meu Perfil
            </Button>
          </>
        )}
    ...
    <Paper elevation={3} sx={{ p: 3, borderRadius: 2, height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {livrosIndicados.length === 0 ? (
        <Box sx={{ flexGrow: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Typography variant="body2" color="text.secondary">Nenhuma indicação para esta semana.</Typography>
        </Box>
      ) : (
      ...
      {usuario.role === "FUNCIONARIO" ? (
        ...
        <Grid container spacing={4}>
          <Grid item xs={12} md={4}>
            {/* Aumentei um pouco a altura para caber os botões extras sem scroll */}
            <Box mb={3} sx={{ minHeight: 450 }}><CardAcessoRapido /></Box>
            <Box sx={{ minHeight: 300 }}><CardDestaques /></Box>
          </Grid>
                    ...
                    {temDados && barData.length > 0 ? (
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={barData} layout="vertical" margin={{ left: 0, right: 20, bottom: 20 }}>
                          ...
                          <RechartsTooltip />
                          <Bar dataKey="qtd" fill="#8884d8" radius={[0, 4, 4, 0]} barSize={30} name="Empréstimos">
